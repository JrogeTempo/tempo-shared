...
export type Transaction = {
  ...
  subject: string | null;
  categories: { name: string; icon: string | null; color: string | null; is_visible: boolean } | null;
  accounts: { name: string; bank: string | null } | null;
...
export type RuleCondition = { type: RuleConditionType; value: string };
export type RuleOperator = 'AND' | 'OR';

export const RULE_CONDITION_LABEL: Record<RuleConditionType, string> = {
  contains: 'contiene las palabras',
  from: 'viene de un remitente que contiene',
  subject: 'tiene un asunto que contiene',
};
...
export type MailRule = {
  ...
  receiver: { id: string; business_name: string; person_name: string | null } | null;
};
...
export type Overview = {
  ...
  daily: Array<{ day: string; total_gs: number; cumulative_gs: number }>;
  trend: Array<{ month: string; total_gs: number }>;
...
export type TempoScore = {
  ...
  horizons: { daily: TempoRate; weekly: TempoRate; cycle: TempoRate };
  payment: {
    ticket_rate: number;
    average_gs: number;
    median_gs: number;
  ...
  components: { base: number; pace: number; frequency: number };
  trend: { direction: 'accelerating' | 'steady' | 'slowing'; weekly_vs_monthly: number };
  target: {
    bpm: number;
    zone: TempoZone;
    pace: number;
  ...
  songs: Song[];
  target_songs: Song[];
  advice: Array<{ key: string; title: string; detail: string; bpm_delta: number }>;
  history: Array<{ month: string; bpm: number; zone: string }>;
};
